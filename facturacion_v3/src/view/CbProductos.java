...
public class CbProductos extends JComboBox<Producto> {
	/**
	 * 
	 */
	...
	public void recargarCombo(Integer idProducto) {
		if (idProducto!=null){
			...
			int pos = 0;
			removeAllItems();
			ArrayList<Producto> lista = new BDDProductos().recuperaPorFiltro("productos.id =" + idProducto);
			if (lista!=null) {
	...
	public void recargarCombo() {
		removeAllItems();
		ArrayList<Producto> lista = new BDDProductos().recuperaPorFiltro(null);
		if (lista!=null) {
			for (Producto p : lista) {
				addItem(p);
			}
	...
	public void recargarComboFiltrado() {
		...
		ArrayList<Producto> lista = new BDDProductos().recuperaPorFiltro(filtroString);
		if (lista!=null) {
			for (Producto p : lista) {
				addItem(p);
			}
