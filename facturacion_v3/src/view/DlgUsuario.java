...
public class DlgUsuario extends JDialog {
	...
	private CbEstados cbStatus;

	//private Usuario userLogin;
	
	public DlgUsuario(Usuario user) {
		this.usuario = user;
		setResizable(false);
		setModal(true);
		...
		if (this.usuario!=null && this.usuario.getId()>0) {
			this.usuario = new BDDUsuarios().recuperaPorId(this.usuario.getId());
		} else {
			this.usuario = null;
		}
		setForm();
	...
	private void eliminar() {
		...
		if (id!=null && id>0) {
			...
			if (pregunta==JOptionPane.OK_OPTION) {
				boolean eliminado = new BDDClientes().eliminar(id);
				if (eliminado) {
					usuario = null;
	...
	private void guardar() {
		...
		if (usuario != null && usuario.getEmail()!= null
			...
			int newId = new BDDUsuarios().grabar(usuario);
			if (newId >= 0) {
				usuario.setId(newId);
	...
	private boolean ponerPassword(String password) {
		...
		if (password!=null && usuario!=null && usuario.isLoged()) {
			BDDUsuarios db = new BDDUsuarios();
