...
public class DlgProducto extends JDialog {
	...
	public DlgProducto(Producto pro) {
		this.producto = pro;
		...
		if (producto!=null && producto.getId()>0) {
			producto = new BDDProductos().recuperaPorId(producto.getId());
		} else {
			producto = null;
		}
		setForm();
	...
	private void eliminar() {
		...
		if (id!=null) {
			...
			if (pregunta==JOptionPane.OK_OPTION) {
				boolean eliminado = new BDDProductos().eliminar(id);
				if (eliminado) {
					producto = null;
	...
	private void guardar() {
		producto = getForm();
		if (producto!=null) {
			int newId = new BDDProductos().grabar(producto);
			if (newId>=0) {
